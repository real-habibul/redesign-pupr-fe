...
export async function login(payload: LoginPayload): Promise<AuthResponse> {
  ...
  const success =
    raw.success ??
    (typeof raw.status === "string" ? raw.status === "success" : !!raw.status) ??
  ...
  return {
    ...
    data: {
      accessToken,
      user,
      // tambahkan map lain kalau perlu
    },
  };
}
...
export async function ssoLogin(payload: SsoPayload): Promise<AuthResponse> {
  ...
  const message = raw.message ?? (success ? "Login SSO berhasil!" : "Login SSO gagal.");
